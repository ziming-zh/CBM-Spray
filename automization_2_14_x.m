...
for pamb=[40,70,100]
    for T=[30,60,90]
        ...
        %     time = 4;
            ...
            [x_max,x_max_index]=max(x);
            [x_min,x_min_index]=min(x);
            [vor_x_val,vort]=min(abs(x(6:x_max_index)));
            vort=vort+4;
            
            % X Fitting
            
            [dif_fit_result_x,dif_fit_eval_x]=diffusionFit(x(1:50),x_max_index);
            
            [ent_fit_result_x,ent_fit_eval_x]=entrainFit(x,x_min_index);
            
            vort=vort+1;
            min_vort = min(6,vort-2);
            
            
            ent_curve=[zeros(min_vort-1,1);ent_fit_result_x(min_vort:vort);zeros(51-(vort),1)]';
            dif_curve=[zeros(vort,1);flipud(dif_fit_result_x(x_max_index+1:x_max_index*2-vort));dif_fit_result_x(x_max_index+1:51)]';
        ...
        %     
        %     x_min_index=0;
            exp_curve_r=[zeros(x_min_index,1);de_fit_result_x(x_min_index+1:vort-1);zeros(52-(vort),1)]';
            ...
            xlabel('y, mm')
            ylabel(['Entrainment Velocity $v_N$, $m/s$'],'interpreter','latex')
